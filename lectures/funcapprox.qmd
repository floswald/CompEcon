...
Let's have a look at the famous [Iris Dataset](https://en.wikipedia.org/wiki/Iris_flower_data_set).
...
* We could think of each row as a _vector_. A *tuple* of input data, like $x_1 = (5.1,3.5,1.4,0.2)$, and a one-dimensional ouput $y_1$, the _species_ label, like "setosa". Stack the $x$ to get $X$ of size (150,4).
...
Those measurements seem to be on different scales. The _SepalLength_ measure seems to have an average of around 5, where as _PetalWidth_ is smaller than 1 in most cases. 
...
What we have done right now, i.e. to *transform* this data via _addition and multiplication_ into a **common space** is very important. In fact, it is related to the concept of **Vector Spaces**. 
...
* If we choose $\Phi$ so that it's elements would share a lot of information, it will be hard to invert it. Basically _multicollinearity_.
...
Here is a nice illustration of spline basis functions from [`BSplineKit`](https://jipolanco.github.io/BSplineKit.jl/dev/generated/approximation/):
...
function plot_knots!(ax, ts; ybase = 0, knot_offset = 0.03, kws...)
    ...
    Makie.scatter!(ax, ts, ys; marker = '×', color = :gray, markersize = 24, kws...)
    ax
end
...
function plot_basis!(ax, B; eval_args = (), kws...)
    ...
    for (n, bi) in enumerate(B)
        ...
        Makie.lines!(ax, ts[i]..ts[j], x -> bi(x, eval_args...); color, linewidth = 2.5)
    end
...
```{julia}
using Interpolations as Itp
A = rand(10,5)
itp = Itp.interpolate(A, Interpolations.BSpline(Quadratic(Reflect(OnCell()))))
itp(1.9,4.9)
```
...
itp = Itp.interpolate(A, Interpolations.BSpline(Cubic(Interpolations.Line(OnGrid()))))
...
knots = (A_x,)
itp = Itp.interpolate(knots, A, Gridded(Linear()))
itp(2.0)
```
...
itp = Itp.interpolate(knots, A, (Gridded(Linear()),Gridded(Constant())))
itp(4,1.2)
```
