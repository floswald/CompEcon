...
# ╔═╡ 4a4938e6-a172-4f2c-b9ec-8d2b140dcd00
...
# ╔═╡ 24dde5e6-0d4a-4282-96a0-377549def6a8
begin
...
# ╔═╡ f5c7919b-7a5c-4ebe-b11c-a8c74add2f45
...
* Return a `Dict` with keys `:beta` (true values), `:n` (number of observations, `:X` an `(n,k)` matrix of data and `:y` a response vector."
...
# ╔═╡ d425ea5a-7ea2-4d5a-aacc-e6f575d68a3b
hint(md"""
Here is how you could generate a matrix of beta values which looks like this, i.e. 
you have `ngrid` copies of the true beta vector.
...
# ╔═╡ 4f4d7418-e9fd-4ed5-87a7-7830cadb75bc
hint(md"""
And here is how you could build the plot with a loop:
...
for ib in 1:k
	betas = copy(beta0)  # copy the above matrix
	# modify the ib-th row to have the [:β][ib] varying
	betas[ib,:] = collect(range(d[:β][ib] - pad, stop = d[:β][ib] + pad, length = ngrid))
	# the `mapslices(x -> loglik(x,d), betas, dims = 1)[:]` thing applies 
	# `loglik` to each slice of `betas`
	p0 = plot(betas[ib,:], mapslices(x -> loglik(x,d), betas, dims = 1)[:], title = "β$(ib-1)",leg = false, lw = 2)
	vline!(p0,[d[:β][ib]], color = :red,lw =2)  # make a vertical line at truth
	push!(pl,p0)
end
plot(pl...,layout = (1,3),size = (700,200))
...
if ismissing(plotLike())
	still_missing(md"modify the body of function `plotLike` above!")
end
...
# ╔═╡ 914f90ad-0969-49a4-8c0a-3418ef959c48
info(text) = Markdown.MD(Markdown.Admonition("info", "Info", [text]))

# ╔═╡ 956fab25-78ac-4027-a125-ad9fbd51ab7e
info(md"""

# Please try!

You should first try yourself to make that plot. The complicated part is to create the data. spend some time on it. If you are stuck, let me provide 2 hints. First hint is for the data structure, second for the plot. Of course the plot corresponds to my data structure.
""")
...
manifest_format = "2.0"
project_hash = "6647738849a06ba450cb5ce63260f66c09373c3e"
