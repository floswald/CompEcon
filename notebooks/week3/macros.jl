...
👉 The problem here is that we would have to evaluate the expression `1 + 2` *before* we call the `time()` function. A function does not take *epxressions* but (references to) value. `time(1 + 2)` would first do `1 + 2`, and then try to time the operation - that obviously does not work. 

We really want to **wrap** this operation into some that which takes the time. Like, we would do
...
You can see that hard coding this each time is cumbersome. Other languages rely on string representations of expressions, with the obvious limitations:
